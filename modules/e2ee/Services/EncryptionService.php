...
use App\Modules\E2ee\Services\BaseE2eeService;
use App\Modules\E2ee\Traits\E2eeConfigTrait;

class EncryptionService extends BaseE2eeService
{
    use E2eeConfigTrait;
    ...
    public function __construct(
        ...
        $this->algorithm = $this->getEncryptionConfig('algorithm', 'AES-256-GCM');
        $this->keyLength = $this->getEncryptionConfig('key_length', 32);
     ...
     */
    public function encryptWithCache(string $data, int $userId, string $cacheKey, int $ttl = 3600): array
    {
        ...
        return Cache::remember($cacheKey, $ttl, function () use ($data, $userId) {
            return $this->encrypt($data, $userId);
        });
    }
     ...
     */
    public function decryptWithCache(string $encryptedData, string $iv, int $userId, string $cacheKey, int $ttl = 3600): string
    {
        $cacheEnabled = config('e2ee.performance.cache_enabled', true);

        if (!$cacheEnabled) {
            return $this->decrypt($encryptedData, $iv, $userId);
        }

        $cacheKey = "e2ee:decrypt:" . hash('sha256', $cacheKey);

        return Cache::remember($cacheKey, $ttl, function () use ($encryptedData, $iv, $userId) {
            return $this->decrypt($encryptedData, $iv, $userId);
        });
    }
     ...
     */
    public function batchEncrypt(array $items, int $userId): array
    {
        $results = [];
        ...
        foreach (array_chunk($items, $batchSize) as $batch) {
            foreach ($batch as $index => $item) {
                try {
                    $results[$index] = $this->encrypt($item, $userId);
                } catch (\Exception $e) {
                    $results[$index] = null;
     ...
     */
    public function batchDecrypt(array $items, int $userId): array
    {
        $results = [];
        $batchSize = config('e2ee.performance.batch_size', 100);
        ...
        foreach (array_chunk($items, $batchSize) as $batch) {
            foreach ($batch as $index => $item) {
                try {
                    if (is_array($item) && isset($item['encrypted_data'], $item['iv'])) {
                        $results[$index] = $this->decrypt($item['encrypted_data'], $item['iv'], $userId);
                    } else {
                        $results[$index] = null;
                        $this->logError('batch_decrypt', "Invalid item format for index {$index}");
                    }
                } catch (\Exception $e) {
                    $results[$index] = null;
     ...
     */
    private function validateInput(string $data, int $userId): void
    {
        if (empty($data)) {
        ...
        if ($userId <= 0) {
            throw new E2eeException('Invalid user ID', 'INVALID_USER_ID');
        }
    }
    ...
    public function testEncryptionCycle(string $testData = null): bool
    {
        try {
            ...
            $encrypted = $this->encrypt($testData, $userId);
            $decrypted = $this->decrypt($encrypted['encrypted_data'], $encrypted['iv'], $userId);
