// @test @model @api-key @deletion
/**
 * Tests for ApiKey model deletion.
 * Ensures deletion logic and soft deletion are covered.
 */

import { describe, it, expect, beforeEach, vi } from 'vitest'
import { createMockApiKey, createMockUser, setupTestEnvironment } from '../helpers/testUtils'
import { MockApiKey } from '../helpers/mockModels'

// Use the mock ApiKey class
const ApiKey = MockApiKey

describe('ApiKey Deletion', () => {
  let mockApiKey: any
  let mockUser: any

  beforeEach(() => {
    vi.clearAllMocks()
    
    mockUser = createMockUser()
    mockApiKey = createMockApiKey({
      user: mockUser,
      user_id: 1,
      name: 'Test API Key',
      key: 'test_api_key_123',
      permissions: ['read', 'write'],
      expires_at: '2025-01-01T00:00:00Z',
      last_used_at: '2024-01-01T12:00:00Z',
      is_active: true
    })
  })

  setupTestEnvironment()

  it('should delete API key', async () => {
    ApiKey.delete.mockResolvedValue(true)
    await ApiKey.delete(1)
    
    expect(ApiKey.delete).toHaveBeenCalledWith(1)
  })

  it('should handle soft deletion', async () => {
    const softDeleteData = {
      is_active: false,
      deleted_at: new Date().toISOString()
    }

    ApiKey.update.mockResolvedValue({ ...mockApiKey, ...softDeleteData })
    await ApiKey.update(1, softDeleteData)
    
    expect(ApiKey.update).toHaveBeenCalledWith(1, softDeleteData)
  })
}) 